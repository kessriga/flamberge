...
- CI (`.github/workflows/ci.yml`) gates every PR: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and build+test on Linux/macOS/Windows. Run `cargo fmt` before committing — warnings and unformatted code fail the merge. Releases: pushing a `v*` tag runs `.github/workflows/release.yml`, which builds an optimized `flamberge` per platform (x86_64 Linux, arm64+x86_64 macOS, x86_64 Windows) and attaches `.tar.gz`/`.zip` archives to the GitHub release. Top-level `LICENSE` is the MIT license; the workspace `license = MIT` (inherited by every crate via `license.workspace = true`).
