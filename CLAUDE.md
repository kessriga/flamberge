...
- **Real + tested:** all of `flamberge-crypto` (PC1, Topaz, AES CBC/ECB/CTR, DES, RC4, CRC-32, PBKDF2, PKCS#7); `flamberge-formats::palmdb`, `mobi` + `topaz_container` (TPZ0 header/payload/metadata parsing) + `ion` (Amazon ION binary pull parser: VarUInt/VarInt, type descriptors, list/struct navigation, symbol table with the `ProtectedData` shared import, annotation resolution) + `kfx_zip` (KFX-ZIP: locate DRMION/voucher members by magic, strip DRMION 8+8, repackage with decrypted members via `raw_copy_file`); `flamberge-keys` offline generators (`pid`, `ignoble`, `ereader`, `kobo::derive_userkeys`); the **Mobipocket, Topaz, and KFX schemes end-to-end** (`flamberge-schemes::mobipocket`, `flamberge-schemes::topaz`, `flamberge-schemes::kfx`, wired through the CLI). Topaz emits a repackaged decrypted `TPZ0` (extension `tpz`); KFX emits a repackaged `.kfx-zip` (voucher unwrap → AES-128-CBC page decrypt + LZMA-alone via `lzma-rs`); HTML/SVG rendering (§5.5–5.6) is out of scope.
- **Stubbed** (return `Unimplemented`, doc-comment points at a spec §): all `flamberge-schemes` decrypt bodies *except mobipocket, topaz, and kfx*, the remaining `flamberge-formats` containers (`ocf`, `pdf`), and platform key-extraction (`kindle`, `adobe`, `kobo::discover_userkeys`).
- **Next vertical slice:** OCF/EPUB (§7 container) — `flamberge-formats::ocf` reader+writer (TASK-8), feeding the Adobe ADEPT (TASK-9) and B&N (TASK-10) EPUB schemes. Note: KDF SQLite (`CONT`) → KFX-ZIP unpacking is out of scope (lives in the external KFX Input plugin); the KFX path here ingests `.kfx-zip` only.
