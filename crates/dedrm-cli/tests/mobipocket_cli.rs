//! End-to-end integration test for the `dedrm` binary against Mobipocket DRM.
//!
//! ## Fixture provenance
//!
...
//! mirrors what `dedrm-schemes::mobipocket` already round-trips in its unit
...
use dedrm_schemes::mobipocket::KEYVEC1;
...
fn temp_dir(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("dedrm-it-{}-{tag}", std::process::id()));
...
fn dedrm() -> Command {
    Command::new(env!("CARGO_BIN_EXE_dedrm"))
}
