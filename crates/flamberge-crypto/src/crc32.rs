...
//! * [`flamberge`] — the `(~binascii.crc32(data, -1)) & 0xFFFFFFFF` variant used by
...
mod tests {
    ...
    #[test]
    fn flamberge_is_uninverted() {
        // flamberge(x) and ieee(x) differ only by the standard init/xorout folding.
        assert_ne!(flamberge(b"123456789"), ieee(b"123456789"));
    }
}
