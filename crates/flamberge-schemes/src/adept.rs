...
use base64::Engine;
use flamberge_crypto::{aes, kdf, rsa};
...
/// Decrypt one encrypted OCF member: AES-128-CBC with IV = the first 16
/// ciphertext bytes over the remainder (equivalent to the reference's "decrypt
/// the whole blob with a zero IV, then drop the first 16 plaintext bytes"), strip
/// PKCS#7, then raw inflate. A member that was stored (not deflated) fails inflate
/// and passes through unchanged (§7.3).
fn decrypt_member(book_key: &[u8; 16], data: &[u8]) -> Result<Vec<u8>> {
    if data.len() < 16 || data.len() % 16 != 0 {
        return Err(invalid(
            "ADEPT encrypted member is not a whole number of AES blocks",
        ));
    }
